...
- (void)layoutSubviews {
    numberOfPoints = [self.delegate numberOfPointsInGraphWithTag:self.tag]; // The number of Points in the Graph.
...
- (void)drawGraph {
    ...
    for (int i = 0; i < numberOfPoints; i++) {
        ...
        float dotValue = [self.delegate valueForIndex:i WithTag:self.tag];
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer {
    ...
    if (closestDot.tag > 99 && closestDot.tag < 1000) {
        if ([self.delegate respondsToSelector:@selector(didTouchGraphWithClosestIndex:WithTag:)])  [self.delegate didTouchGraphWithClosestIndex:((int)closestDot.tag - 100) WithTag:self.tag];
    }
    
    // ON RELEASE
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        if ([self.delegate respondsToSelector:@selector(didReleaseGraphWithClosestIndex:WithTag:)]) [self.delegate didReleaseGraphWithClosestIndex:(closestDot.tag - 100) WithTag:self.tag];
...
- (float)maxValue {
    ...
    for (int i = 0; i < numberOfPoints; i++) {
        dotValue = [self.delegate valueForIndex:i WithTag:self.tag];
...
- (void)drawXAxis {
    if (![self.delegate respondsToSelector:@selector(numberOfGapsBetweenLabelsWithTag:)]) return;
    ...
    int numberOfGaps = [self.delegate numberOfGapsBetweenLabelsWithTag:self.tag] + 1;
    ...
    if (numberOfGaps >= (numberOfPoints - 1)) {
        ...
        firstLabel.text = [self.delegate labelOnXAxisForIndex:0 WithTag:self.tag];
        ...
        lastLabel.text = [self.delegate labelOnXAxisForIndex:(numberOfPoints - 1) WithTag:self.tag];
        ...
        for (int i = 1; i <= (numberOfPoints/numberOfGaps); i++) {
            ...
            labelXAxis.text = [self.delegate labelOnXAxisForIndex:(i * numberOfGaps - 1 - offset) WithTag:self.tag];
